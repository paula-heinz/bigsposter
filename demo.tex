...
%
We need to achieve pin positions at the same relative border positions and equivalent pin graphs on the  reuse blocks $B_1,...,B_k$, to be able to stamp the routing from the leader to the followers and save the routing passes on the followers.
...
\item[\raisebox{0.8cm}{1.}] \begin{minipage}{0.5\blockwidth}
...
Pin intervals and extra reuse blockages solve that problem.
\end{minipage}
\hfill
...
\textbf{Reuse Blockages:}\label{sec:reuseblockages}
...
The yellow border blockages in the northern vertical gap and the pink border blockages in the middle gap overlap in a T-junction,
where no routing is allowed.
We need to copy an equivalent blockage to the right (blue) reuse block to prevent pins in that area.


\blocktitle{Pin Intervals}\label{sec:pinintervals}
%TODO fix notation, less introduction
%TODO Add picture from reduction
...
\blocktitle{Multiple equivalence classes of reuse blocks}\label{sec:multiple-equivalent classes}
