...
\end{minipage}
 ...
 {\footnotesize \begin{spacing}{0.9}
	...
	\begin{itemize}
		...
		is within a factor $(1+\epsilon)$ of the source-sink distance, (usually $\epsilon=.1$)
		\item If a (partial) port assignment is given, assigned pins are connected over the assigned port positions.
	\end{itemize}
	We achieve this using a two step Steiner tree oracle for the nets in  global routing.
	...
	In the reuse setting (see Section~\ref{sec:reuse}), we might be given a partial port assignment, i.e. predefined input port positions for some blocks. %TODO {Das muss spaeter noch besser beschrieben werden}
	
	
	Then, we compute a  blockage-avoiding approximate shortest path $r$-arborescence $Y_{top}$ with the cluster roots as sinks.
	
	After that we traverse the vertices $v\in V(Y_{top})$ in reverse topological order.
	...
	$Y$ is the union of all these arborescences.
	
	To embed $Y$ into the 3-dimensional global routing graph, we traverse the vertices $v\in V(Y)$  bottom-up and embed sibling paths $\delta^+(v)$ until they meet near the position of $v$ (see \cite{BRGTiming2}). This embeddding considers the congestion prices and sink delay prices provided by the resource sharing algorithm (see \cite{BRGTiming2}).
	...
	In case of rectangular blocks this avoids zig-zagging along block borders. This embedding is timing- and congestion-aware\\
\end{blockrow}
 \begin{blockrow}
...
If two terminals  in block  are part of a net $N$, their equivalent terminals in other reuse blocks must also be connected
...
%
We need to achieve port positions at the same relative border positions and equivalent port graphs on the  reuse blocks $B_1,...,B_k$, to be able to use the same routing for all reuse blocks.
...
We first choose an interval (often just a point) on the boundary such that the worst detour w.r.t. the  $\ell_1$-metric among all equivalent blocks in an individual net routing is minimized. This is a simple geometric problem.
This interval is then increased  by a constant value to give the router flexibility in avoiding congestion. To roughly align routing in the first pass, we shrink large intervals to some maximal length.
...
\end{minipage}
\vspace{0.3cm}\\
%It could be preferable to allow $k>1$ pin positions  per net, e.g. if a root in side a reuse block
has sinks north and south of a block, but has not been necessary yet on our instances.
...
\begin{tabular}{cc}
	...
	%TODO fix this
		  ...
		  Port intervals (green) from step \ref{PIstep} and reuse blockages from step \ref{PIstep} ensure that reuse blocks have similar, good and feasible port positions after the first routing pass, which is needed to determine the feedthroughs through reuse blocks.
...
The second routing pass with track assignment (only necessary at the borders of the leader) legalizes  the port positions the position from the first pass without moving them too much.
