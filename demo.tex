...
\begin{document}
  ...
  \begin{blockrow}[3]
    \blocktitle{Introduction}
    	...
    	Pangea is tailored but not limited to a setting, where the
...
Equivalent reuse blocks can be mirrored on the x- and y- axis with respect to each other (indicated by {\sffamily F}).
...
\item \begin{minipage}{0.5\blockwidth}
...
\begin{tikzpicture}
	...
	\node[] (r1) at (0.1,0.7) {};
	\draw [orange] (r1)--(0.4, 0.7) --(s1);
	\draw [orange] (r1) -- (3.55, 0.7) -- (3.55, -0.4)--(s11);
	\draw [orange, dashed] (1.85,0.7) -- (1.85, -0.4) --(s1);
...
\item Secondly, we need to reflect that  floating terminals  must be connected to pins and create pins  for \textit{feedthroughs}, i.e.\
...
\begin{tabular}{cc}
	\begin{minipage}{0.5\blockwidth}
		  ...
		  Thus, pin positions are relaxed to intervals. 	\end{minipage}    &\begin{minipage}{0.49\blockwidth}
...
\blocktitle{Pin Intervals}\label{sec:pinintervals}
...
\blocktitle{Multiple equivalence classes of reuse blocks}
