...
\title{Congestion-\& Timing-Aware Port Assignment with Reuse Blocks}
\posteryear{2025}
...
If the net connects to sinks inside a reuse block, multiple port positions, could still be interesting but would
require to add a multiplexer between pins and sinks in each block.

In this case choosing the best port positions becomes a kind of facility location problem on the boundary.
...
\raggedright
{\footnotesize \begin{spacing}{0.9}
		Possible routing without port intervals. When the port position of this routing from the leader is forced on the follower, a huge detour is caused (dashed).\end{spacing}	}
\end{minipage}
