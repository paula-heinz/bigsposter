...
A port assignment of a net and port graph specifies for each inner vertex of the port graph. A partial port assignment only assigns port positions to some blocks. %TODO
...
We compute port positions (or more precisely port graph parts) on one arbitrarily chosen reuse block $B_1$ that we call the \textit{leader} (solid line). 
...
\begin{tabular}{cc}
	...
	%TODO fix this
		  ...
		  Steps~\ref{step:add-blockages} and \ref{PIstep} ensure that reuse blocks have similar and good and feasible port positions in the first routing pass, which is needed to determine the feedthroughs through reuse blocks.
