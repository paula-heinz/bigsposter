...
\begin{document}
  \maketitle
  \begin{blockrow}
    \blocktitle{Introduction}
    Large integrated circuits are often partitioned into multiple blocks
    to reduce tool running times and  parallelize the design process. Pin
    ...
    Further  complications arise from  multiply-instantiated reuse blocks.
    We present a new method for congestion- and timing-aware pin assignment that can
    handle symmetries due to multiply-instantiated reuse blocks.
  ...
  \blockbreak
  \blocktitle{Routing Modifications  for Pin Assignement}
  ...
  \blocktitle{Border Blockages \& Adjusted Tiles}
  ...
  block boundaries on all layers whose preference direction is parallel
  to the boundary.
  ...
  The global router has to  suppin non-uniform tiles.
...
\end{blockrow}
 \begin{blockrow}
 	\blocktitle{Hierarchical Topologies}
...
In this section, we explain how we deal with mupliply instantiated reuse blocks.
For simpler exposition, we assume that there is only a single set of equivalent reuse blocks, say  $B_1, ...,B_k$  and ordinary non-reuse blocks $B_{k+1},\dots, B_n$.
In Section~\ref{sec:multiple-equivalent classes}, we show how to generalize this to multiple equivalence classes.
%TODO we require that they are not neighboring...
%TODO say that wlog all are inter block...
Equivalent reuse blocks can be mirrored on the x- and y- axis with respect to each other.
We assume that we are given an equivalence relation on the terminals in different blocks $B_i$ ($i\in [k]$).
If two terminals  in block $B_i$ are part of a inter-block net $N$, their equivalent mates in another block must be connected
...
%
The goal of this section is to describe how we achieve pin positions at the same relative border positions and equivalent pin graphs
on the  reuse blocks $B_1,...,B_k$.

We compute pin positions (or more precisely pin graph parts) on one arbitrarily chosen reuse block $B_1$ that we call the \textit{leader}.
Then, we  force these positions on the other reuse blocks $B_1\dots, B_k$, called the \textit{followers}. 
For that to work correctly, we need to consider two things: 

%TODO make this shorter
Firstly, pin positions that are good  for the leader might  cause huge detours or, due to obstacles around the block borders, even be infeasible for the followers.
Section \ref{sec:reuseblockages} addresses how we achieve feasibility and in Section \ref{sec:pinintervals} we cover how to avoid unnecessarily large detours.
...
Secondly, we need to reflect that  floating terminals  must be connected to pins and create pins  for \textit{feedthroughs}, i.e.\
...
\begin{algorithm}[H]
	\begin{enumerate}
		\item add in blockages and reuse blockages\\ (Sections~\ref{sec:tiles_n_border_blockages} \& \ref{sec:reuseblockages})\label{step:add-blockages}
		\item choose pin intervals (Section~\ref{sec:pinintervals}) \label{PIstep}
		\item \textbf{first routing pass w/o track assignment} \label{step:pass1}
		\item for each  feedthrough  through a reuse block,
		create an artificial net with artifial pins on the boundary of each equivalend block.
		Similarly, add virtual nets connecting floating terminals with the boundary.
	...
	\label{alg:reuse-alg}
\end{algorithm}
Steps~\ref{step:add-blockages} and \ref{PIstep} ensure that reuse blocks have the same set of feasible positions and use pin intervals imposing  similar positions in routing (see Figures~\ref{fig:3pass1}).
Using pin positions of the first routing pass, we  align feedthroughs and connectiong to floating terminals across reuse blocks in Step~\ref{step:create-artificial-feedthroughs}.
The simple copying of pin positions of  articficial  nets on the borders  may leave overlaps.
Thus, pin positions are relaxed to intervals.

The next routing call with track assignment legalizes  the pin positions the position from the first pass.
Now, we enforce the legal pin positions from the leader on the followers in Step~\ref{step:copy-pin-assignment}.
Step~\ref{step:add-blockages} guarantees that  they are legal for the followers.

Finally, the third routing pass is  adapts pin assignment on non-reuse blocks to the new forced pin positions on the followers.
The running time  is dominated by the three routing passes. 


Note that with above flow we get dead feedthroughs, i.e. extra virtual
nets not needed for signal transmission.
%
To reduce the number of dead feedthroughs, it is possible to match
feedthroughs of different blocks, but this is beyond the scope of this
paper.

\subsection{Reuse Blockages}\label{sec:reuseblockages}
As  pin positions are  copied between reuse blocks, pin positions that are infeasible on one block should also be infeasible for others.
We align the blockage structure around equivalent blocks, by transferring blockages in the corridors around each reuse block to its equivalent counterparts.
In Figure~\ref{fig:3pass1}, the yellow border blockages in the northern vertical gap and the pink border blockages in the middle gap overlap in a T-junction,
where no routing is allowed.
We need to copy an equivalent blockage to the right (blue) reuse continent to prevent pins in that area.


\subsection{Pin Intervals}\label{sec:pinintervals}
%TODO fix notation, less introduction

Simply forcing the pin assignment from the leader to a follower could lead to an unnecessary detour on the follower block (see orange net in Figure \ref{fig:3pass1}). 
Also the routing on the follower and its neighbors could change significantly, after the pins assignment is imposed.
...
%% \begin{comment}
	...
	\centering
		...
		%blocks
		\draw[thick,blue] (0,0) rectangle (\reusewidth,0-\rowheightl);
		...
		\fill[gray] (\uppersmallwidth-0.1, \corridorwidth+0.1) rectangle (\uppersmallwidth+0.1+\corridorwidth, -0.1);
		\fill[gray] (2*\reusewidth+2*\corridorwidth +\otherwidth-\uppersmallwidth+0.1, \corridorwidth+0.1) rectangle (2*\reusewidth+\corridorwidth +\otherwidth-\uppersmallwidth-0.1, -0.1);
		...
		\node[] (r11) at (0.1,0.68) {};
		\draw [orange] (r11)--(0.4, 0.68) --(s1);
		\draw [orange] (r11) -- (3.55, 0.68) -- (3.55, -0.4)--(s11);
		\draw [orange, dashed] (1.85,0.68) -- (1.85, -0.4) --(s1);
...
\end{tikzpicture}
\subsection{Dealing with multiple equivalence classes}
\label{sec:multiple-equivalent classes}

So far we considered  a single equivalence class of reuse blocks.
In case of multiple equivalence classed things become more complicated.
