...
\textbf{Embedding into Global Routing Graph}	\\
	To embed $Y$ into the 3-dimensional global routing graph, we traverse the vertices $v\in V(Y)$  bottom-up and embed sibling paths $\delta^+(v)$ until they meet near the position of $v$. This embeddding considers the congestion prices and sink delay prices provided by the resource sharing algorithm (see \cite{BRGTiming2}).
	It decides the  layers and wire configuration that greatly determine the signal speed and possibly introduces detours to 
